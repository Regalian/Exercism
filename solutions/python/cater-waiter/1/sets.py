...
def check_drinks(drink_name: str, drink_ingredients: list[str]) -> str:
    ...
    return drink_name + (
        " Cocktail" if ALCOHOLS.intersection(drink_ingredients) else " Mocktail"
    )
...
def categorize_dish(dish_name: str, dish_ingredients: set[str]) -> str:
    ...
    for category, ingredients in FOOD_CLASSES.items():
        if dish_ingredients <= ingredients:
            return dish_name + ": " + category
    return dish_name + " : UNKNOWN"


def tag_special_ingredients(dish: tuple[str, set[str]]) -> tuple[str, set[str]]:
...
def compile_ingredients(dishes: list[tuple[str, set[str]]]) -> set[str]:
...
def singleton_ingredients(dishes: list[set[str]], intersection: set[str]) -> set[str]:
    ...
    for dish in dishes:
        singletons.update(dish - intersection)
    return singletons
